...
var resizePizzas = function(size) {
    ...
    function determineDx(elem, size, oldWidth, windowWidth) {
        //Performance(BWF) - Moved these assignments to the top of the calling function to remove them from the loop.
    ...
    function changePizzaSizes(size) {
        ...
        for (var i = 0; i < randPizzaContainerLength; i++) {
            var dx = determineDx(randPizzaContainer[i], size, oldWidth, windowWidth);
            var newwidth = (oldWidth + dx) + 'px';
            randPizzaContainer[i].style.width = newwidth;
        }
    }

    /* Performance(BWF) - Refactored changePizzaSize. See above.
      function changePizzaSizes(size) {
...
function updatePositions() {
    ...
    var scrollTopTemp = document.body.scrollTop / 1250;
    for (var i = 0; i < items.length; i++) {
        var phase = Math.sin((scrollTopTemp) + (i % 5));
